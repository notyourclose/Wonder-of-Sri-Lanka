  ...
  return (
    <>
      <motion.section
        ...
        className={`py-16 md:py-24 px-4 w-full ${reverse ? 'bg-gray-50' : 'bg-white'}`}
      >
        <div className={`container mx-auto flex flex-col ${reverse ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-8 md:gap-12`}>
          <motion.div 
            variants={imageVariants} 
            className="w-full md:w-1/2 transform-gpu"
          >
            <img
              src={imgSrc}
              alt={title}
              className="rounded-lg shadow-lg w-full h-56 sm:h-72 md:h-[400px] object-cover"
            />
          </motion.div>
          <motion.div 
            variants={itemVariants} 
            className="w-full md:w-1/2 space-y-4 md:space-y-6 transform-gpu"
          >
